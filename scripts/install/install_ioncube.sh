...
install_ioncube() {
     ...
     if [[ "${PHP_VERSION}" == "7.2" ]]; then
          ...
          # For PHP 8.x releases the path format remains consistent.  Write
          # the loader directive into both FPM and CLI configurations.
          echo "zend_extension=$MODULES/ioncube_loader_lin_${PHP_VERSION}.so" >>/etc/php/${PHP_VERSION}/fpm/php.ini
          echo "zend_extension=$MODULES/ioncube_loader_lin_${PHP_VERSION}.so" >>/etc/php/${PHP_VERSION}/cli/php.ini
     else
